...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    // "Archive Your Memory" buradan kaldırıldı, üst kısma taşındı kanka [cite: 2026-03-01]
    ...
    <nav className="fixed top-0 left-0 w-full z-[1000] p-4 md:p-10 bg-black/90 backdrop-blur-sm border-b border-white/5">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-6">
          ...
          {/* --- Buton menüden dışarı taşındı --- */}
          <Link href="/submit" className={`hidden md:block text-[9px] tracking-[0.3em] uppercase transition-all duration-300 border border-white/20 px-4 py-2 hover:bg-white hover:text-black ${pathname === '/submit' ? 'bg-white text-black' : 'text-white'}`}>
            Archive Your Memory
          </Link>
        </div>
      ...
      <div className={`md:hidden flex flex-col items-center gap-6 pt-10 transition-all duration-300 ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        {/* Mobil menüde de buton görünecek */}
        <Link href="/submit" className={`text-[10px] tracking-[0.4em] uppercase transition-all duration-300 hover:text-white ${pathname === '/submit' ? 'text-white' : 'text-white/70'}`} onClick={() => setIsOpen(false)}>
            Archive Your Memory
        </Link>
        {navLinks.map((link) => (
